...
int main(void)
{
    ...
    fputs("set xlabel \"v_{refL} [V]\"  font \"BKM-cmr10,30\" offset 0,-1.5\n", pipe);
    fputs("set ylabel \"dI/dV_{refL} [mS]\" font \"BKM-cmr10,30\" offset -8,0\n", pipe);
    fputs("set key font\"Arial,25\"\n", pipe);
    fputs("set key bottom left spacing 2.5 offset 22,0\n", pipe);
...
//    fputs("set label 52 center at 0.189 , 1.31  \"mV\"          font \"BKM-cmr10, 20\" \n", pipe);
    ...
    fputs("FT = \"Vref_IL_FT.vcsv\" \n", pipe);
    fputs("TT = \"Vref_IL_TT.vcsv\" \n", pipe);
    fputs("ST = \"Vref_IL_ST.vcsv\" \n", pipe);

    fputs("plot   FT skip 6 using 1 : ($4) * 1e3 with lines title \"FT\" \n", pipe);
    fputs("replot TT skip 6 using 1 : ($4) * 1e3 with lines title \"TT\" \n", pipe);
    fputs("replot ST skip 6 using 1 : ($4) * 1e3 with lines title \"ST\" \n", pipe);
