...
\section{三端子間に同時に寄生容量がついた場合の小信号等価解析}
    ...
    折り返し型のギルバート乗算回路でシミュレーションを行い、.op解析からコントロール電圧毎のパラメータを抽出した。この時のシミュレーション条件を図\refeq{table:state_folded_ac}に示す。また、この時のシミュレーション波形と.op解析で得られたパラメータを代入した理論式のグラフを図\refeq{fig:vout_theoretical}、\refeq{fig:vout_sim}に示す。
    \begin{table}[h]
...
\section{負荷にインダクタを負荷した際の周波数特性}
    ...
    \end{multline}
    と求められた。コントロール電圧を$200\,\mathrm{mV}$に固定し、それ以外の条件は\refeq{table:state_folded_ac}と同様の条件でインダクタを付加した際の\eqref{eq:vout_inductor}式から得られる周波数特性を図\refeq{fig:vout_inductor_theoretical}に、シミュレーション波形を図\refeq{fig:vout_inductor_simulation}に示す。
    \begin{figure}[H]
        \begin{center}
