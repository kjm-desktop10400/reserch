...
\section{負荷にインダクタを負荷した際の周波数特性}
    ...
    \end{multline}
    と求められた。コントロール電圧を$200\,\mathrm{mV}$に固定し、それ以外の条件は表\refeq{table:state_folded_ac}と同様の条件でインダクタを付加した際の\eqref{eq:vout_inductor}式から得られる周波数特性を破線で、シミュレーション波形を実践で図\refeq{fig:inductor}に示す。ただし、どちらにおいても最も下の線が$10\,\mathrm{nH}$であり、上に行くにしたがって$5\,\mathrm{nH}$づつ大きくなる。
    \begin{figure}[H]
        \begin{center}
