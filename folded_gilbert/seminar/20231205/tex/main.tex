...
\section{カレントミラーのサイズによる利得への影響}
    ...
    \begin{align}
        \Delta v=\frac{4K_{A}R_{L}\Delta g_{m}}{1+2R_{L}g_{dA}}\cdot\frac{g_{mMA}}{g_{mMB}}\cdot\frac{g_{mB}}{g_{mA}}\cdot V_{CTRL}\cdot v_{in}     \label{eq:vout}
    \end{align}
...
\section{素子値の設計}
    ...
    $\mathrm{M_{A}}$には後述する整合の影響を受け$\pm0.1\;\mathrm{V}$の信号が入る。図\refeq{fig:nmos_unit}では$0.7\;\mathrm{V}$付近から線形に電流が増加していたので、ゲートソース間電圧を$0.8\;\mathrm{V}$付近でで利用することとした。また、$\mathrm{M_{A}}$のソース電位は$0.3\;\mathrm{V}$とした。この時ゲートのバイアス電圧$\mathrm{V_{lbias}}$は$1.1\;\mathrm{V}$と決まる。さらに、飽和領域で使用するためゲート電位はドレイン電位よりもしきい値電圧分大きくなればよいので、今回は少し余裕を持たせ、ドレイン電位は$0.7\;\mathrm{V}$とした。さらに$M_{B}$についても$M_{A}$と同様$V_{AS}=0.3\;\mathrm{V}$、
