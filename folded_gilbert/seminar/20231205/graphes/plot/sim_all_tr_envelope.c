...
int main(void)
{
    ...
    fputs("set ytics 0.05 offset 0 , 0\n", pipe);
    fputs("set mxtics 10\n", pipe);
    fputs("set mytics 5\n", pipe);
    fputs("set grid xtics mxtics ytics linewidth 2, linewidth 1, linewidth 1\n", pipe);
    fputs("set xrange [0 : 100]\n", pipe);
    fputs("set yrange [-0.25 : 0.25]\n", pipe);
    #pragma endregion
    ...
    */
    ...
    fputs("input = \"..\\\\data\\\\sim_all_tr_envelope.vcsv\"\n", pipe);

    for(int i = 1; i*0.02 <= 0.31; i++)
    {
        if(i == 1)
        {
            fprintf(pipe, "plot input skip 6 using (($%d) * 1e9) : (($%d) * 1e0) with lines notitle #\"%.2f V\" \n", 2 * i - 1, 2 * i, i * 0.02);
        }
        else 
        {
