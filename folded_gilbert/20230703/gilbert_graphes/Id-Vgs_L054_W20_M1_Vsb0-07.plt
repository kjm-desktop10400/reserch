...
plot   input skip 6 using 1  : (($2 ) * y_scale) with lines title "Vsb : 0   V"
replot input skip 6 using 5  : (($6 ) * y_scale) with lines title "Vsb : 0.2 V"
replot input skip 6 using 3  : (($4 ) * y_scale) with lines title "Vsb : 0.7 V"

#f1(x) = A * x + B
#f2(x) = C * x + D
#f3(x) = E * x + F
#
#fit [0.69 : 0.71] f1(x) input skip 6 using 1 : (sqrt($2 ) * y_scale) via A, B
