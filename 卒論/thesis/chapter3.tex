    ...
    \section{シミュレーションによる確認}
        ...
        \subsection{素子値の設計}
            ...
            \begin{align*}
                ...
                0.9 &< V_{out}
            \end{align*}
            と分かり、$V_{out}-\frac{1}{2}v_{out}=0.9\;\mathrm{V}$とした。直流電流についてはとりあえず$1\;\mathrm{mA}$とした。これはドレインソース間電圧が$0.4\;\mathrm{V}$程度でも十分流せる電流だと考え決めた。出力端子の直流電位は$\mathrm{M_{A}}$のドレイン端子の下限$0.9\;\mathrm{V}$と電源電圧$1.8\;\mathrm{V}$の中心である$1.35\;\mathrm{V}$になるよう、無入力状態では負荷抵抗にドレイン電流の半分が流れることに注意し、$R_{L}=900\;\mathrm{\Omega}$とした。この直流バイアスを満たすようなトランジスタサイズを表\ref{table:3_previous}に示す。
