...
int main(void)
{
    ...
    fputs("set ylabel \"trance impedance [k ohm]\" font \"Arial,30\" offset -8,0\n", pipe);
    ...
    */
    ...
    fputs("input = \"..\\\\data\\\\doubleStageTiaAc.vcsv\"\n", pipe);

    fprintf(pipe, "plot     input skip 6 using 1 : (sqrt(($2)**2 + ($3)**2)) * 1e-3 with lines title \"out\" \n");
    fprintf(pipe, "replot   input skip 6 using 1 : (sqrt(($5)**2 + ($6)**2)) * 1e-3 with lines title \"v_{m}\" \n");
