...
int main(void)
{
    ...
    fputs("set mytics 5\n", pipe);
    fputs("set grid xtics mxtics ytics linewidth 2, linewidth 1, linewidth 1\n", pipe);
    fputs("set xrange [0 : 0.5]\n", pipe);
    fputs("set yrange [-0.6 : 0.6]\n", pipe);
    #pragma endregion
    ...
    */
    ...
    fputs("input = \"..\\\\data\\\\bufferTran.vcsv\"\n", pipe);

    fprintf(pipe, "plot     input skip 6 using ($1)*1e9 : ($2) with lines title \"v_{in}\" \n");
    fprintf(pipe, "replot   input skip 6 using ($3)*1e9 : ($4) with lines title \"v_{out}\" \n");
