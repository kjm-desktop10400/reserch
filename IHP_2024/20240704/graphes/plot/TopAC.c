...
int main(void)
{
    ...
    fputs("set key bottom left spacing 2.5 offset 30,5\n", pipe);
    ...
    */
    ...
    for(int i = 1; i <= 10; i++)
    {
        ...
        fprintf(pipe, "input skip 6 using 1 : ( sqrt( ($%d)**2 + ($%d)**2 ) * 1e-3 ) with lines notitle \" Vctrl = %.2lf V \" ", 3*(i - 1) + 2, 3*(i - 1) + 3, i * 0.01);
